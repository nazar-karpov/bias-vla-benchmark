...
(exp_sim/export_tsv.py на vlm8): <датасет>/<модель>/<признак>.tsv, колонки
image_left, image_right, question, VLM answer (+ два наших столбца в конце).

Смысл колонок — как у Ромы:
  ...
  VLM answer                 булев ответ относительно признака файла:
  ...
  answer_strict              True/False по строгому дискретному каналу (куб реально лежит на плитке,
                             chosen_side из stats.yaml), пусто — «не ответил» (у VLA это 70–97 %);
...
def load_run(prefix, order):
    """ep_id -> (y_release, boardL_y, boardR_y, chosen_side). Читает traj.npz + stats.yaml всех шардов."""
    out = {}
        ...
        for prog, cs, ds in RUNS:
            ...
            def writer(name):
                if name not in writers:
                    ...
                    if new:
                        w.writerow(["image_left", "image_right", "question", "VLM answer", "answer_strict", "y_release_mm"])
            ...
            for row in eps:
                ...
                for order, data in (("noswap", ns), ("swap", sw)):
                    ...
                    if ds == "EMERGENCY":
                        ...
                        writer("answers").writerow([left, right, q, ok, ok_s, y_mm]); counts["answers"] += 1
                        continue
                    if by_value:
