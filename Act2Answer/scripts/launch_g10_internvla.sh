...
#   NODE=SMOKE: сервер на GPU0 + 4 эпизода pairs_g10 (проверка перед боем)
...
chain() {  # gpu  "ASSETS:ORDER:START0:END" ...
  ...
  setsid -f nohup bash -c "$cmds" < /dev/null > "$LOGS/chain_g10_internvla_${NODE}_gpu${gpu}.out" 2>&1
  echo "GPU$gpu: $*"
}
...
case "$NODE" in
  SMOKE)
    ...
    python -u -m simpler_env.eval --vla internvla --vla-path "$CKPT" --assets pairs_g10 \
