#!/usr/bin/env bash
# H100: полный confirm RLDX, диапазон [START, START+COUNT), noswap+swap.
# Клиент (magma_act2answer env) к УЖЕ ПОДНЯТОМУ RLDX-серверу на RLDX_PORT (:20000).
set -uo pipefail
...
GPU_A2A="${GPU_A2A:-0}"
NAMEPFX="${NAMEPFX:-confirm-rldx-full-h100}"
