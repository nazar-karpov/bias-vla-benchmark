...
ENV_PATH=${CONDA_ENVS_DIR}/spatialvla_act2answer
LOG=${A2A_LOG_DIR}/spatialvla_setup.log
...
exec > >(tee -a "$LOG") 2>&1
echo "START_SPATIALVLA_SETUP $(date -u)"
...
retry pip4 install -r "$REPO_ROOT/requirements/spatialvla.txt"
retry pip4 install -e "$REPO_ROOT/ManiSkill" -e "$REPO_ROOT/SimplerEnv"

python - <<'PY'
import torch, transformers
print("spatialvla_env_ok", torch.__version__, transformers.__version__, "cuda", torch.cuda.is_available())
PY
echo "DONE_SPATIALVLA_SETUP $(date -u)"
