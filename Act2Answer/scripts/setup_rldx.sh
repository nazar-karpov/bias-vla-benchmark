...
if [ ! -x "$REPO/.venv/bin/python" ]; then
  uv sync --python 3.10 || { echo "uv sync failed"; exit 1; }
fi
uv pip install -e . || { echo "uv pip install -e . failed"; exit 1; }

# 2. sanity import
uv run python -c "import rldx; from rldx.policy.server_client import PolicyServer; print('rldx_import_ok', rldx.__version__)" \
...
CKPT_DIR="$HOME/bias_benchmark/nazar_folder/rldx_ckpt/RLDX-1-FT-SIMPLER-WIDOWX"
if [ ! -f "$CKPT_DIR/.download_done" ]; then
  mkdir -p "$CKPT_DIR"
  uv run python - "$CKPT_REPO" "$CKPT_DIR" <<'PY'
import sys
