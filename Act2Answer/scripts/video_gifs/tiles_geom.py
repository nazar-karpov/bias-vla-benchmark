...
зоны ответа (hard = половина bbox + 1 см = ±0.076, soft = + 3 см = ±0.096 от центра). С размером 0.132 гомография не сходится (невязка до
30 px), с 0.174 — ≤5 px. Видимая плитка — ±0.087: soft-зона шире её лишь на ~0.9 см, а hard-зона на ~1.1 см уже.
...
SOFT, HARD = 0.096, 0.076                 # зоны ответа из кода среды: bbox 0.11 × 1.2 / 2 + 0.03 / + 0.01
