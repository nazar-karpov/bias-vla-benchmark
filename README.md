...
## ⭐ Итог (сентябрь 2026): что мы в итоге нашли

Линия измерений завершена **пре-регистрированными confirm-прогонами**
(гипотезы и правило решения коммитились в гит ДО запуска):

- **Magma — 4 подтверждённых стереотипных эффекта** (n≈800 пар/ячейку, 8 сидов,
  BH-поправка): *receptionist→женщина* (−7.2 мм, q=8e−05), *poor→чёрный* (−5.9,
  q=.0063), *flight attendant→женщина* (−5.6, q=.023), *athlete→чёрный* (−5.3,
  q=.024). Величина: ≈0.1 SD эпизода, но ~20% всего семантического отклика модели.
  Все — «негативная/низкостатусная роль → маргинализованная группа»; на престижных
  ролях (CEO, pilot, professor) эффектов нет.
- **InternVLA — подтверждённого bias НЕТ** при живом понимании вопроса
  (полярный гейт p=3e−10): читает инструкцию, демографической тяги не имеет.
- ⇒ **Bias — свойство конкретной модели, а не VLA как класса** (из 8 прогнанных
  моделей эффектами обладают единицы).
- Методический бонус: показано вживую, как позиционный артефакт + отбор по данным
  производят 12 «эффектов» из которых настоящих 4 (слот → центр → confirm),
  и величины при подтверждении падают вдвое (winner's curse).

**Куда смотреть** (порядок чтения для нового человека/агента):

| файл | что это |
|---|---|
| [`docs/METRICS_ALL_MODELS.md`](docs/METRICS_ALL_MODELS.md) | сводка всех метрик по всем моделям + финальная confirm-секция — **начинать отсюда** |
| [`docs/JOURNAL.md`](docs/JOURNAL.md) | повествовательный журнал: почему метод такой, тупики, смены методики (свежие сверху) |
| [`EXPERIMENTS.md`](EXPERIMENTS.md) | таблица всех прогонов (№40–46 — финальная single-card/confirm линия) |
| [`docs/CONFIRM_CENTER4.md`](docs/CONFIRM_CENTER4.md), [`docs/CONFIRM_INTERNVLA_DID.md`](docs/CONFIRM_INTERNVLA_DID.md) | пре-регистрации confirm-прогонов (гипотезы/правила ДО запуска) |
| [`docs/METRIC_DESIGN.md`](docs/METRIC_DESIGN.md) | дизайн метрик и связь с литературой |
| [`metrics/`](metrics/) | текстовые сводки каждого анализа (confirm_center4_full.txt — финал Magma) |
| `Act2Answer/scripts/` | все скрипты: раннеры `run_single_card_center.sh`, `run_confirm_*.sh`; анализ `single_card_assent.py`, `confirm4_analysis.py` |
| [`docs/PAPER.md`](docs/PAPER.md), `icml_paper/` | заготовки статьи — следующий этап |
