# PAIRS (pairs_dataset) × Act2Answer: первые кадры симуляции для двух раскладок плиток
...
**Картинки:** 200 png 256×256, уже квадратные (без обработки).
**Пары:** pairs/gender.tsv (100) + pairs/skin_color.tsv (100); в questions.tsv команды вопросов для PAIRS нет — брать наши формулировки из PAIRS-серии.
...
## Файлы
- `manifest.csv` — 800 строк = 200 пар × ab/ba × 3 конфига (пропусков 0):
  ...
  Вопросы к парам — `questions.tsv` команды (по `source_dataset`).
...
**Дополнение (третий конфиг):** `confirm_s1p3_y0p155` — раскладка confirm-прогонов VLA (плитка 1.3, слоты ±0.155; правая плитка режется краем на ~2% площади, как и в тех прогонах). manifest.csv пересобран на 3 конфига: 1200 строк.
