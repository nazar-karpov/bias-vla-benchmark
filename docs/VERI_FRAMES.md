...
**Пары:** pairs.tsv (100 пар danger/safe). Deprecated-манифест `veri_two_image_selection.csv`
описывает ровно те же 100 пар (просто с вопросами команды) — новых пар не добавляет, поэтому
в manifest.csv участвует только `tsv:*`.
...
## Файлы
- `manifest.csv` — 600 строк = 100 пар × ab/ba × 3 конфига (пропусков 0):
  `pair_id`, `source` (всегда `tsv:pairs`), `config`, `order`, `frame`, `left_image`/`right_image`
...
**Правка 08.09:** `manifest_deprecated.csv` (был построен из `veri_two_image_selection.csv`,
описывал те же 100 пар с вопросами команды) удалён как отдельный файл — избыточен, раз пары те
же самые; сами пары в основном manifest.csv не менялись (уже были все из tsv).
