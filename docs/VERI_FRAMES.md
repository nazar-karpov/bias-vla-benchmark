...
- `manifest.csv` — 600 строк = 100 пар × ab/ba × 3 конфига (пропусков 0):
  ...
  (все колонки таблицы пар). Вопросы к парам — в `questions.tsv` рядом с кадрами (155 штук: 150 из общей таблицы
  команды + 5 из veri_emergency). Задаются ВСЕ вопросы ко ВСЕМ парам всех датасетов:
  колонки `source_dataset` и `source_format` — только происхождение вопроса (из какой
  задачи исходного датасета он вырос), НЕ фильтр; все 155 уже переписаны командой под
  парный дизайн («Put the cube…» / «Which image, A or B…»). В манифест вопросы НЕ
  развёрнуты — кадр от вопроса не зависит, полный крест это ~12 млн строк на 4 датасета.
